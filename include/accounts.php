...
class stafflog {
	protected static  $tblname = "staff";
	...
	static function userAuthentication($U_USERNAME,$h_pass){
		global $mydb;
		$mydb->setQuery("SELECT * FROM `staff` WHERE `username` = '". $U_USERNAME ."' and `pass` = '". $h_pass ."'");
		$cur = $mydb->executeQuery();
		if($cur==false){
			die(mysql_error());
		}
		$row_count = $mydb->num_rows($cur);//get the number of count
		 if ($row_count == 1){
		 $user_found = $mydb->loadSingleResult();
		 	$_SESSION['USERID']   		= $user_found->staff_id;
		 	$_SESSION['U_NAME']      	= $user_found->name;
		 	$_SESSION['U_USERNAME'] 	= $user_found->username;
		 	$_SESSION['U_PASS'] 		= $user_found->pass;
		   return true;
		 }else{
		 	return false;
		 }
