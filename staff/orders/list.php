			    ...
			    <form action="controller.php?action=delete" Method="POST">  					
				 <div class="table-responsive">	
                  <table id="example" class="table  table-striped table-bordered table-hover"  style="font-size:12px" cellspacing="0">
					...
					<?php 
					$hay=$_SESSION['USERID'];
				  		/*$query = "SELECT * FROM `tblsummary` s , `tblcustomer` c , `tblproduct` p ,`shop` sh ,`staff` s , `tblorder` or 
				  				WHERE  s.`CUSTOMERID`=c.`CUSTOMERID` AND s.`ORDEREDNUM`=or.`ORDEREDNUM` AND or.`PROID`=p.`PROID` AND p.`shop_id` = sh.`shop_id` AND s.`staff_id` = $hay AND s.`shop_id`=sh.`shop_id`";
				  		*/
						$query = "SELECT * FROM `tblsummary` s ,`tblcustomer` c , `tblorder` ord , `tblproduct` p
				  				WHERE   s.`CUSTOMERID`=c.`CUSTOMERID` AND s.`ORDEREDNUM`=ord.`ORDEREDNUM` AND ord.`PROID`=p.`PROID` ";
