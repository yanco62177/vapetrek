		...
		<div class="row">
        	...
        	<!-- /.col-lg-12 -->
			  ...
			  <tbody>
				  	<?php 
					$hay=$_SESSION['USERID'];
				  		$query = "SELECT * FROM `tblpromopro` pr , `tblproduct` p , `tblcategory` c , `shop` sh , `staff` s
           					 WHERE pr.`PROID`=p.`PROID` AND  p.`CATEGID` = c.`CATEGID` AND  p.`shop_id` = sh.`shop_id` AND s.`staff_id` = $hay AND s.`shop_id`=sh.`shop_id`";
				  		$mydb->setQuery($query);
				    ...
				    echo '<td style="padding:0px;">
				  		...
				  		echo '<td><a title="edit" href="'.web_root.'admin/products/index.php?view=edit&id='.$result->PROID.'">'.$result->PRODNAME.'</a></td>';
